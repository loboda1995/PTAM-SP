...
typedef nanoflann::KDTreeSingleIndexAdaptor<nanoflann::L2_Simple_Adaptor<double, PointCloud>, PointCloud, 2> ImageRefKD;
typedef nanoflann::KDTreeSingleIndexDynamicAdaptor<nanoflann::L2_Simple_Adaptor<double, PointCloud>, PointCloud, 3> ModelPointsKD;
...
struct Level {
    inline Level() : keypointsKD(2, keypointsPC, nanoflann::KDTreeSingleIndexAdaptorParams(10 /* max leaf */)){
    ...
    PointCloud keypointsPC;
    ImageRefKD keypointsKD;
...
struct KeyFrame {
    ...
    std::vector<cv::KeyPoint> relocKeypoints;
    std::map<int, cv::Point3d> relocWorldPoints;
    cv::Mat relocFrameDescriptor;
    void MakeKeyFrame_Reloc(int featureCount, double maxPointRadius);
