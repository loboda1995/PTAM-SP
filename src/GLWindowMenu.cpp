...
void GLWindowMenu::GUICommandHandler(std::string sCommand, std::string sParams) {
    ...
    if (sCommand == msName + ".AddMenuToggle") {
        if (vs.size() < 3) {
            std::cout
                    << "? GLWindowMenu.AddMenuToggle: Need 3/4 params: Target Menu, Name, gvar_int name , NextMenu=\"\""
                    << std::endl;
            return;
        };
        MenuItem m;
    ...
    if (sCommand == msName + ".AddMenuSlider") {
        if (vs.size() < 5) {
            std::cout
                    << "? GLWindowMenu.AddMenuSlider: Need 5/6 params: Target Menu, Name, gvar_int name, min, max, NextMenu=\"\""
                    << std::endl;
            return;
        };
        MenuItem m;
        ...
        case Slider: {
            if (nMouseButton == GLWindow::BUTTON_WHEEL_UP) {
                ...
                *(SelectedItem.gvnIntValue) =
                        (int) (dFrac * (1.0 + SelectedItem.max - SelectedItem.min)) + SelectedItem.min;
            };
        }
            break;
        case Monitor:
